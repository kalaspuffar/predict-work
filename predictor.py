...
#            .view(np.float64).reshape(disk_days_sa.shape + (-1,))
        ...
        # Extract only smart_* attributes from the structured array into a 2D array
        smart_attrs = [attr for attr in model_smart_attr if 'smart_' in attr]
        disk_days_attrs = np.stack([disk_days_sa[attr] for attr in smart_attrs], axis=-1)
